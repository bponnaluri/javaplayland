window.inject = (d) ->
	...
	<p class="rp">&nbsp;&nbsp;&nbsp;&nbsp;Loops can be used to simplify longer processes. For example, the array created in a previous example can be changed so that setting the array values and printing them can be done in only a couple lines</p>

	<div class="ex">int[] numArray = new int[3];\nfor(int i=0; i<numArray.length; i++) {\n\tnumArray[i] = i\n\tprint(numArray[i])\n}</div>
